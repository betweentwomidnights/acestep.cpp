...
static struct ggml_tensor * dit_adapter_linear_transform(struct ggml_context * ctx,
    ...
    if (output_offset > 0 || weight->ne[1] != found->first->ne[1]) {
        ...
        if (adapter.dora_rows) {
            adapter.magnitude = ggml_view_1d(
                ctx, adapter.magnitude, weight->ne[1], output_offset * adapter.magnitude->nb[0]);
            adapter.base_norm_sq = ggml_view_1d(
                ctx, adapter.base_norm_sq, weight->ne[1], output_offset * adapter.base_norm_sq->nb[0]);
        }
    }
    ...
    struct ggml_tensor * cross_term = ggml_sum_rows(
        ctx, ggml_cont(ctx, ggml_transpose(ctx, ggml_mul(ctx, weight_t_a, b_t))));
    ...
    struct ggml_tensor * norm_sq    = ggml_add(
        ctx,
        ggml_add(ctx,
